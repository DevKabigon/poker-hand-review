...
export async function generateMetadata({
  params,
}: OddsPageProps): Promise<Metadata> {
  const { locale } = await params;
  const currentLocale = locale as AppLocale;
  const t = await getTranslations({ locale, namespace: "toolsHub" });
  const title = t("oddsTitle");
  const description = t("oddsDescription");

  return {
    title,
    description,
    alternates: {
      canonical: localizePathname(currentLocale, "/tools/odds"),
      ...localizedAlternates("/tools/odds"),
    },
    openGraph: {
      title,
      description,
      url: localizedAbsoluteUrl(currentLocale, "/tools/odds"),
...
export default async function OddsToolPage() {
            ...
            <Card className="rounded-4xl border border-border/75 bg-card p-6 shadow-[var(--shadow-soft)] sm:p-8">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h1 className="mt-2 text-3xl font-black tracking-tight">{tTools("oddsTitle")}</h1>
                  <p className="mt-2 text-sm text-muted-foreground sm:text-base">
                    {tTools("oddsDescription")}
                  </p>
                </div>
